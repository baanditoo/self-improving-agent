...
def parse_token(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "quote_mint": payload.get("quoteMint") or payload.get("quote_mint") or "",
        "quote_category": payload.get("quoteCategory") or payload.get("category") or "",
        "launchpad": launchpad,
        "status": status,
        "stage": stage,
        "mode": payload.get("mode") or "",
        "market_cap_usd": _num(payload.get("marketCap")) or _num(market.get("marketCap")),
        "volume_usd": _num(payload.get("volume")) or _num(payload.get("volume24h")) or _num(market.get("volume")),
        "price_usd": _num(payload.get("priceUsd")) or _num(payload.get("price")) or _num(market.get("priceUsd")),
        "liquidity_usd": _num(payload.get("liquidity")) or _num(payload.get("liquidityUsd")) or _num(market.get("liquidity")),
